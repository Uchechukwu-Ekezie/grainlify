...
pub enum Error {
    ...
    InvalidAssetId = 33,
    /// Anonymous escrow: refund requires resolution via authorized resolver
    AnonymousRefundRequiresResolution = 34,
    /// Caller is not the configured anonymous resolver
    NotAnonymousResolver = 35,
    /// Escrow is not anonymous; use standard refund
    NotAnonymousEscrow = 36,
    /// Anonymous resolver not configured
    AnonymousResolverNotSet = 37,
    /// Use get_escrow_info_v2 for anonymous escrows (depositor is commitment)
    UseGetEscrowInfoV2ForAnonymous = 38,
}

/// Identifies a party either by plain address or by a 32-byte commitment (hash).
/// Used for optional anonymization: store only the commitment on-chain and resolve
/// the real address off-chain via an authorized backend or zero-knowledge proof.
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
...
pub enum DataKey {
    ...
    /// Anonymous escrow (bounty_id -> AnonymousEscrow) when depositor is stored as commitment only
    EscrowAnon(u64),

    /// Address authorized to resolve anonymous refunds (calls refund_resolved with recipient)
    AnonymousResolver,
}
...
impl BountyEscrowContract {
    ...
    /// Set or clear the anonymous resolver (admin only).
    /// The resolver is the only address that can call `refund_resolved` to direct
    /// refunds for escrows created with `lock_funds_anonymous`.
